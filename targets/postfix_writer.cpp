...
void til::postfix_writer::processIDPBinaryExpression(
...
void til::postfix_writer::processGeneralLogicalBinaryExpression(
    cdk::binary_operation_node *const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  node->left()->accept(this, lvl + 2);
...
void til::postfix_writer::do_declaration_node(til::declaration_node *const node,
  ...
  if (node->initializer()) {
    if (_inFunctionBody)
      processLocalVariableInitialization(symbol, node->initializer(), lvl);
    else
      processGlobalVariableInitialization(symbol, node->initializer(), lvl);
...
void til::postfix_writer::do_program_node(
  ...
  _pf.TEXT("_main"); // !!! todo: check if should be "main"
  _pf.ALIGN();
